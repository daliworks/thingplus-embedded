...
static bool _discoverable_parse(json_object *json, char *key)
{
	...
	if (!json_object_object_get_ex(json, key, &discoverable_object)) {
		tube_log_error("[TUBE_HTTP] json_object_object_get_ex failed. key : %s\n", key);
		return false;
	}
...
static int _gw_info_model_parse(struct json_object* json)
{
	...
	if (json_object_object_get_ex(json, "model", &model_object) == false) {
		tube_log_error("[TUBE_HTTP] json_object_object_get_ex failed. key : model\n");
		return -1;
	}
...
static void _gw_info_device_add(struct gateway_info *gw_info, char *device_id)
{
	...
	if (gw_info->devices == NULL) {
		tube_log_error("[TUBE_HTTP] realloc failed. size:%d\n",
			realloc_size);
		return;
	}
...
static int _gw_info_get(struct tube_http *t)
{
	...
	if (url == NULL) {
		tube_log_error("[TUBE_HTTP] tube_http_url_get failed. gw_id:%s\n", t->gateway_id);
		return -1;
	}

	if (tube_curl_read(url, t->curl, (void*)&t->gw_info.p) < 0) {
		tube_log_error("[TUBE_HTTP] tube_curl_read(TUBE_HTTP_URL_GATEWAY_INFO) failed. gw_id:%s\n", t->gateway_id);
...
static void _device_model_parse(struct device_model *device_model, struct json_object *device_model_object)
{
	...
	if (device_model->sensors == NULL) {
		tube_log_error("[TUBE_HTTP] calloc failed. size:%ld\n", sizeof(struct sensor));
		return;
	}
...
static int _gw_model_parse(struct tube_curl_payload *p, struct gateway_model *gw_model)
{
	...
	if (json_object_object_get_ex(p->json, "deviceModels", &device_models_objects) == false) {
		tube_log_error("[TUBE_HTTP] json_object_object_get_ex failed. key : deviceModels\n");
		return -1;
	}
...
static int _gw_model_get(struct tube_http *t)
{
	...
	if (url == NULL) {
		tube_log_error("[TUBE_HTTP] tube_http_url_get failed\n");
		return -1;
	}

	if (tube_curl_read(url, t->curl, &t->gw_model.p) < 0) {
		tube_log_error("[TUBE_HTTP] tube_curl_read(TUBE_HTTP_URL_GATEWAY_MODEL) failed\n");
		tube_http_url_put(url);
		return -1;
	}
...
{
	if (id_template == NULL) {
		tube_log_error("[TUBE_HTTP] id_template is NULL");
		return false;
	}
...
static int _device_register(struct tube_http* t, struct device_model *device_model, char *name, char* device_id)
{
	...
	if (tube_curl_post(url, t->curl, (void*)json_object_to_json_string(postfields_ojbect), &p) < 0) {
		tube_log_error("[TUBE_HTTP] tube_curl_post(TUBE_HTTP_URL_DEVICE_REGISTER) failed\n");
...
{
	memset(sensor_id, 0, TUBE_HTTP_MAX_ID);
...
static int _sensor_register(struct tube_http *t, char* name, int uid, char* device_id, struct sensor* sensor, char* sensor_id)
{
	...
	if (!id_template) {
		tube_log_error("[TUBE_HTTP] id_template get failed\n");
		return -1;
	}
	...
	struct tube_curl_payload p;
	if (tube_curl_post(url, t->curl, (void*)json_object_to_json_string(postfields_ojbect), &p) < 0) {
		tube_log_error("[TUBE_HTTP] tube_curl_post(TUBE_HTTP_URL_SENSOR_REGISTER) failed\n");
...
static struct device_model* _device_model_get_by_device_id(struct tube_http* t, char* device_id)
{
	...
	if (tube_curl_read(url, t->curl, (void*)&p) < 0) {
		tube_log_error("[TUBE_HTTP] tube_curl_read(TUBE_HTTP_URL_DEVICE_INFO) failed\n");
...
enum tube_http_error tube_http_sensor_register(void* tube_http, char* name, int uid, char* type, char* device_id, char* sensor_id)
{
	...
	if (t == NULL) {
		tube_log_error("[TUBE_HTTP] tube_http is NULL\n");
		return TUBE_HTTP_ERROR_INVALID_ARGUMENT;
	}

	if (name == NULL) {
		tube_log_error("[TUBE_HTTP] name is NULL\n");
		return TUBE_HTTP_ERROR_INVALID_ARGUMENT;
	}

	if (type == NULL) {
		tube_log_error("[TUBE_HTTP] type is NULL\n");
		return TUBE_HTTP_ERROR_INVALID_ARGUMENT;
	}

	if (device_id == NULL) {
		tube_log_error("[TUBE_HTTP] device_id is NULL\n");
		return TUBE_HTTP_ERROR_INVALID_ARGUMENT;
	}

	if (sensor_id == NULL) {
		tube_log_error("[TUBE_HTTP] sensor_id is NULL\n");
		return TUBE_HTTP_ERROR_INVALID_ARGUMENT;
	}

	if (t->gw_info.model == -1) {
		if (_gw_info_get(t) < 0) {
			tube_log_error("[TUBE_HTTP] _gw_info_get failed\n");
			return TUBE_HTTP_ERROR_READ;
		}
	}

	if (!t->gw_info.discoverable) {
		tube_log_error("[TUBE_HTTP] this gateway is not discoverable\n");
		return TUBE_HTTP_ERROR_NOT_DISCOVERABLE;
	}

	if (t->gw_model.nr_device_models == 0) {
		if (_gw_model_get(t) < 0) {
			tube_log_error("[TUBE_HTTP] _gw_model_get failed\n");
	...
	if (dev_model == NULL) {
		tube_log_error("[TUBE_HTTP] no device\n");
		return TUBE_HTTP_ERROR_READ;
	}

	if (!dev_model->discoverable) {
		tube_log_error("[TUBE_HTTP] device is not discoverable\n");
		return TUBE_HTTP_ERROR_NOT_DISCOVERABLE;
	}
	...
	if (_is_sensor_duplicated(&t->gw_info, sensor_id)) {
		tube_log_warn("[TUBE_HTTP] sensor is already registered. name:%s\n", name);
		return TUBE_HTTP_ERROR_DUPLICATED;
	}

	if (_sensor_register(t, name, uid, device_id, sensor, sensor_id) < 0) {
		tube_log_error("[TUBE_HTTP] _sensor_register failed. name:%s\n", name);
...
enum tube_http_error tube_http_device_register(void* tube_http, char* name, int uid, char* device_model_id, char* device_id)
{
	...
	if (!t->gw_info.discoverable) {
		tube_log_error("[TUBE_HTTP] this gateway is not discoverable\n");
		tube_log_error("[TUBE_HTTP] gateway model number is %d\n",
			t->gw_info.model);
		return TUBE_HTTP_ERROR_NOT_DISCOVERABLE;
	}
...
void tube_http_cleanup(void* tube_http)
{
	...
	if (t == NULL) {
		tube_log_warn("tube_http is NULL\n");
		return;
	}
