...
void _mqtt_values_send(void* m)
{
	...
	vs[0].nr_value = 10;
	vs[0].value = calloc(1, sizeof(struct tube_thing_value));
	for(i=0; i<10; i++) {
		vs[0].value[i].value = "13";
		vs[0].value[i].time_ms = (time(NULL) - 60*(10-i)) * 1000;
	}

	vs[1].id = "temperature-000011112222-0";
	vs[1].nr_value = 10;
	vs[1].value = calloc(1, sizeof(struct tube_thing_value));
	for(i=0; i<10; i++) {
		vs[1].value[i].value = "26.4";
		vs[1].value[i].time_ms = (time(NULL) - 60*(10-i)) * 1000;
	}

	tube_mqtt_values_send(m, 2, vs);
}
...
int main(void)
{
	...
	tube_mqtt_status_send(m, 3, things);
	_mqtt_values_send(m);
	while (1);
	/*
	...
	tube_mqtt_single_value_send(m, "humidity-000011112222-0", &v);
	*/
