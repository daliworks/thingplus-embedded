...
int readPacket(Client* c, Timer* timer) 
{
  ...
  if (rem_len > 0 && (c->ipstack->mqttread(c->ipstack, c->readbuf + len, rem_len, left_ms(timer)) != rem_len)) {
    ERROR_LOG( "mqttread the rest for the buffer fail\n");
    goto exit;
  }
...
exit:
  if (rc >= 0) {
    char printbuf[1024];
    TRACE_LOG("Rc %d from receiving packet %s\n", rc, MQTTFormat_toServerString(printbuf, sizeof(printbuf), c->readbuf, len));
  }
  return rc;
}
...
int MQTTConnect(Client* c, MQTTPacket_connectData* options)
{
  ...
  if ((len = MQTTSerialize_connect(c->buf, c->buf_size, options)) <= 0) {
    ERROR_LOG("connect fail=%d\n", len);
    goto exit;
  }
  if ((rc = sendPacket(c, len, &connect_timer)) != SUCCESS)  { // send the connect packet
    ERROR_LOG("send fail=%d\n", rc);
    goto exit; // there was a problem
  }
  ...
  if (waitfor(c, CONNACK, &connect_timer) == CONNACK)
  {
    ...
    if (MQTTDeserialize_connack((unsigned char*)&sessionPresent, &connack_rc, c->readbuf, c->readbuf_size) == 1) {
      INFO_LOG("CONNACK status=[%s]\n", ConnAckErrors[connack_rc]);
      rc = connack_rc;
    } else {
      rc = FAILURE;
    }
...
exit:
  if (rc == SUCCESS) {
    ...
    ERROR_LOG("failure to connect=%d\n", rc);
  }
  return rc;
}
